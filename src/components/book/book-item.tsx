...
const BookItem = ({ book }: BookItemProps) => {
  return (
    <div key={book.isbn} className="flex gap-2 items-center border p-1.5 rounded-md hover:bg-gray-100 cursor-pointer">
      {
        book.thumbnail ?
        <img src={book.thumbnail} alt={book.title} className="h-20 w-14 object-cover rounded-md" /> :
